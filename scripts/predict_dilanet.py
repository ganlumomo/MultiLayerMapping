...
traversability = {
  #0 : 0, # void/other
  0 : 0, # building   128 0   0
  1 : 0, # vegetation 128 128 0
  2 : 0, # sky        128 128 128
  3 : 0, # car        64  0   128 
  4 : 0, # signate    192 128 128
  5 : 1, # road       128 64  128
  6 : 0, # pedestrain 64  64  0
  7 : 0, # fence      64  64  128
  8 : 0, # pole       192 192 128
  9 : 1, # sidewalk   0   0   192
  10 : 0 # cyclist    0   128 192
}
...
def batch_predict_kitti(dataset_name):
    ...
    img_folder="/home/ganlu/image_2/"
    img_list_file="namelist_small.txt"
    save_folder="/home/ganlu/preprocess_data/"
    ...
    for img_id in xrange(len(test_img_list)):
        ...
        rgb_img_file=img_folder+test_img_list[img_id]
	...
	saved_bw_img_file=save_folder+"label_2d_img/"+test_img_list[img_id]
        saved_bin_file=save_folder+"label_2d_bin/"+test_img_list[img_id].replace(".png",".bin")
        # print saved_bin_file
        caffe_in = np.zeros(input_dims, dtype=np.float32)
	rgb_image = cv2.imread(rgb_img_file, 1)
        image = rgb_image.astype(np.float32) - dataset.mean_pixel
        ...
        traversability_prediction = np.empty_like(prediction)
	for i in xrange(prediction.shape[0]):
	  for j in xrange(prediction.shape[1]):
	    traversability_prediction[i][j] = traversability[prediction[i][j]]
	#color_image = dataset.palette[traversability_prediction.ravel()].reshape(image_size)
        #color_image = cv2.cvtColor(color_image, cv2.COLOR_RGB2BGR)
        # print('Writing', saved_color_img_file)
	#cv2.addWeighted(rgb_image, 0.5, color_image, 0.5, 0, color_image)
        #cv2.imwrite(saved_color_img_file, color_image)
        
        cv2.imwrite(saved_bw_img_file, traversability_prediction.astype(np.uint8))  # save as label ID image
