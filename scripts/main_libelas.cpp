...
// Stereo parameters for KITTI odometry
float Tx = -3.798145000000e+02;
float depth_scaling = 1000;
float depth_fact = -Tx * depth_scaling;
uint16_t min_point = std::numeric_limits<uint16_t>::min();
uint16_t max_point = std::numeric_limits<uint16_t>::max();

using namespace std;

// compute disparities of pgm image input pair file_1, file_2
void process (const char* file_1, const char* file_2, std::string depth_name) {
  ...
  // disparity to depth
  cv::Mat depth_img = cv::Mat(height, width, CV_16UC1);
  uint16_t *depth_img_data = reinterpret_cast<uint16_t *>(&depth_img.data[0]);
  for (int32_t i = 0; i < width * height; ++i) {
    float disp = D1_data[i];
    float depth = depth_fact / disp;
    if (depth <= 0)
      depth_img_data[i] = min_point;
    else if (depth > max_point)
      depth_img_data[i] = min_point;
    else
      depth_img_data[i] = (uint16_t)(depth);
  }
  cv::imwrite(depth_name, depth_img);
  std::cout << "Processing: " << depth_name << std::endl;
...
int main (int argc, char** argv) {
  ...
  int img_num = 1100;
  std::string sequence_folder = "/media/ganlu/PERL-SSD/data_odometry_color/dataset/sequences/06/";
  std::string left_img_folder = sequence_folder + "image_2/";
  std::string right_img_folder = sequence_folder + "image_3/";
  std::string depth_img_folder = sequence_folder + "depth_img/";
  
  for (int img_id = 0; img_id <= img_num; ++img_id) {
    char img_id_c[256];
    sprintf(img_id_c, "%06d", img_id);
    std::string img_id_s(img_id_c);

    cv::Mat left_img_png = cv::imread(left_img_folder + img_id_s + ".png", cv::IMREAD_GRAYSCALE);
    cv::Mat right_img_png = cv::imread(right_img_folder + img_id_s + ".png", cv::IMREAD_GRAYSCALE);
    cv::imwrite("left.pgm", left_img_png);
    cv::imwrite("right.pgm", right_img_png);
    process("left.pgm", "right.pgm", depth_img_folder + img_id_s + ".png");
  }
